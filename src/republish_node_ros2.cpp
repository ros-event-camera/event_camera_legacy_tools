...
int main(int argc, char ** argv)
{
  ...
  if (type == "dvs") {
    event_camera_legacy_tools::Republish<dvs_msgs::msg::EventArray> rep(node.get());
    rclcpp::spin(node);  // should not return
  } else if (type == "prophesee") {
    event_camera_legacy_tools::Republish<prophesee_event_msgs::msg::EventArray> rep(node.get());
    rclcpp::spin(node);  // should not return
  } else if (type == "event_packet") {
    event_camera_legacy_tools::Republish<event_camera_msgs::msg::EventPacket> rep(node.get());
    rclcpp::spin(node);  // should not return
  } else {
