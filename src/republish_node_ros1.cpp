...
#include "event_camera_legacy_tools/republish_ros1.h"
...
int main(int argc, char ** argv)
{
  ...
  if (type == "dvs") {
    event_camera_legacy_tools::Republish<dvs_msgs::EventArray> node(pnh);
    ros::spin();
  } else if (type == "prophesee") {
    event_camera_legacy_tools::Republish<prophesee_event_msgs::EventArray> node(pnh);
    ros::spin();
  } else if (type == "event_packet") {
    event_camera_legacy_tools::Republish<event_camera_msgs::EventPacket> node(pnh);
    ros::spin();
  } else {
