  ...
  rosbag2_cpp::Writer * outBag, event_camera_codecs::Encoder * enc) -> size_t
{
  ...
  for (const auto & e : inMsg->events) {
    enc->encodeCD(
      (rclcpp::Time(e.ts).nanoseconds() - firstEventTime.nanoseconds()), e.x, e.y, e.polarity);
  }
  enc->flush();
...
auto main(int argc, char ** argv) -> int
{
  ...
  const size_t numEvents = event_camera_legacy_tools::process_bag(inBag, outBag, topics);
